...
//funciona direitinho!!
int main() {
    int n = 5; //variável para armazenar os inteiros
    ...
    int novo_n = 8;
    int *teste = (int*) realloc(vetor, novo_n * sizeof(int));//testando realloc

    if(teste == NULL) {
        printf("Erro na realocação!");
        return 1;
    } else {
        vetor = teste; //atualiza o ponteiro de vetor!
    ...
    printf("Valores do vetor:\n");
    for (int i = 0; i < novo_n; i++) {
        printf("%d ", vetor[i]); //testar agora com a realocação!!
    }
